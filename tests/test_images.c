...
test_generation(void)
{
	...
	static const char *unsupported[] = {
		"input_fidelity", "moderation", "output_compression",
	...
	result = image_prepare_generation("{", 1, &output, error,
	    sizeof(error));
	CHECK(result == IMAGE_RESULT_INVALID);
	CHECK(strstr(error, "invalid JSON") != NULL);
	result = image_prepare_generation("[]", 2, &output, error,
	    sizeof(error));
	CHECK(result == IMAGE_RESULT_INVALID);
	...
	result = image_prepare_generation(
	    ...
	    "\"stream\":true}") - 1, &output, error, sizeof(error));
	CHECK(result == IMAGE_RESULT_INVALID);
	...
	result = image_prepare_generation(
	    ...
	    sizeof("{\"model\":\"image-model\",\"prompt\":\"\"}") - 1,
	...
	result = image_prepare_generation(
	    ...
	    "\"response_format\":\"url\"}",
	    sizeof("{\"model\":\"image-model\",\"prompt\":\"x\","
	    "\"response_format\":\"url\"}") - 1, &output, error,
	    sizeof(error));
	CHECK(result == IMAGE_RESULT_INVALID);
	for (index = 0; index < sizeof(unsupported) /
	    sizeof(unsupported[0]); index++) {
		CHECK(snprintf(input, sizeof(input),
		    "{\"model\":\"image-model\",\"prompt\":\"x\","
		    "\"%s\":null}", unsupported[index]) < (int)sizeof(input));
...
test_edit_success(void)
{
	...
	CHECK(multipart_file(&multipart, "image[]", "second.webp",
	    "image/webp", second, sizeof(second)) == 0);
	...
	CHECK(strncmp(json_string_value(json_object_get(entry, "image_url")),
	    "data:image/webp;base64,", sizeof("data:image/webp;base64,") - 1)
	    == 0);
	json_decref(root);
	return 0;
}
    ...
    int include_model, int include_image, const char *expected)
{
	...
	if (multipart_field(&multipart, "prompt", "edit") == -1)
		goto nomem;
	if (include_image && multipart_file(&multipart, "image", "input.png",
	    "image/png", image, sizeof(image)) == -1)
		goto nomem;
	if (extra_name != NULL && multipart_field(&multipart, extra_name,
	    extra_value) == -1)
		goto nomem;
...
test_edit_errors(void)
{
	...
	result = image_prepare_edit("application/json", "{}", 2, &output,
	    error, sizeof(error));
	CHECK(result == IMAGE_RESULT_INVALID);
...
test_oversized_edit(void)
{
	...
	memcpy(body + sizeof(header) - 1 + image_length, trailer,
	    sizeof(trailer) - 1);
	result = image_prepare_edit("multipart/form-data; boundary=large",
	    body, total, &output, error, sizeof(error));
	free(body);
	CHECK(result == IMAGE_RESULT_INVALID);
